...
public class AdPlatformSelectorUseCaseTests
{
    ...
    public async Task LoadDataAsync_ValidStream_ReturnsSuccess()
    {
        ...
        _repositoryMock.Setup(r => r.LoadDataAsync(stream)).ReturnsAsync(true);
        ...
        Assert.Null(error);
        _repositoryMock.Verify(r => r.LoadDataAsync(stream), Times.Once());
    }
    ...
    public async Task LoadDataAsync_RepositoryFails_ReturnsFailure()
    {
        ...
        var input = "Яндекс.Директ:/ru";
        using var stream = new MemoryStream(Encoding.UTF8.GetBytes(input));
        _repositoryMock.Setup(r => r.LoadDataAsync(stream)).ReturnsAsync(false);
    ...
    public async Task LoadDataAsync_FormatException_ReturnsErrorAndLogs()
    {
        ...
        var input = "Invalid";
        using var stream = new MemoryStream(Encoding.UTF8.GetBytes(input));
        _repositoryMock.Setup(r => r.LoadDataAsync(stream)).ThrowsAsync(new FormatException("Invalid format"));
    ...
    public async Task LoadDataAsync_UnexpectedException_ReturnsErrorAndLogs()
    {
        ...
        var input = "Яндекс.Директ:/ru";
        using var stream = new MemoryStream(Encoding.UTF8.GetBytes(input));
        _repositoryMock.Setup(r => r.LoadDataAsync(stream)).ThrowsAsync(new Exception("Unexpected error"));
    ...
    public async Task GetPlatformsAsync_EmptyLocation_ReturnsError()
    {
        ...
        // Act
        var (data, error) = await _useCase.GetPlatformsAsync(location);

        // Assert
        Assert.Null(data);
        Assert.Equal("Query data is empty", error);
        _repositoryMock.Verify(r => r.GetPlatformsAsync(It.IsAny<string>()), Times.Never());
    }
    ...
    public async Task GetPlatformsAsync_UnknownLocation_ReturnsError()
    {
        ...
        _repositoryMock.Setup(r => r.GetPlatformsAsync(location)).ReturnsAsync((LocationPlatformsEntity?)null);
        ...
        _repositoryMock.Verify(r => r.GetPlatformsAsync(location), Times.Once());
    }
    ...
    public async Task GetPlatformsAsync_ValidLocation_ReturnsData()
    {
        ...
        _repositoryMock.Setup(r => r.IsDataLoaded).Returns(true);
        _repositoryMock.Setup(r => r.GetPlatformsAsync(location)).ReturnsAsync(entity);
    ...
    public async Task GetPlatformsAsync_Exception_ReturnsErrorAndLogs()
    {
        ...
        var location = "/ru";
        _repositoryMock.Setup(r => r.IsDataLoaded).Returns(true);
        _repositoryMock.Setup(r => r.GetPlatformsAsync(location)).ThrowsAsync(new Exception("Unexpected error"));
